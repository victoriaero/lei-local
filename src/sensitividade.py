...
PIXEL_PERTURB_STD = 0.75   # controla intensidade da mutação no latent
...
SIGMA_INIT = 1.0  # mantido por compatibilidade, mas não é usado na versão local
...
# Se TARGET_CLASS < 0, usamos a classe predita para x0 como alvo
TARGET_CLASS = -1  # -1 = usar classe predita automaticamente

# Pesos da fitness de sensibilidade
SENS_ALPHA_DELTA_P   = 1.0   # quão forte maximizamos |p(z) - p0|
SENS_BETA_LAT_DIST   = 0.1   # penalização de distância em latent
TRUST_REGION_RADIUS  = 3.0   # raio máximo "confortável" ao redor de z0 (norma L2)
TRUST_REGION_PENALTY = 1.0   # penalidade por sair do raio

# Desvio da população local em torno de z0
SIGMA_LOCAL = 0.5
...
# ===========================
# 2.1. Codificar imagem de entrada → latente (z0) e obter p0
# ===========================
...
def get_classifier_prob_and_class(pil_img: Image.Image) -> tuple[float, int]:
    """
    Aplica o classificador ViT-CIFAR-10 à imagem de entrada.
    Retorna (p0, classe_predita).
    """
...
def mutate_latent(z: torch.Tensor, gen: int) -> torch.Tensor:
    ...
    onde max_std = PIXEL_PERTURB_STD * fator, e fator vai de 1.0 → 0.05 ao longo das gerações.
    """
...
# ===========================
# 4. Avaliação de Fitness de Sensibilidade (vetorizado por batch)
# ===========================
...
def evaluate_fitness_sensitivity(
    ...
    """
    LEI-Local (Sensibilidade):
    - Maximiza |p_c(z) - p0| (mudança na probabilidade da classe alvo)
    - Penaliza distância em latent em relação a z0
    - Penaliza sair da trust-region (raio TRUST_REGION_RADIUS)
    Retorna:
      fitness          : [N]
      delta_p_all      : [N]  (|p_c(z) - p0|)
      dist_lat_all     : [N]  (||z - z0||)
      p_target_all     : [N]  (p_c(z))
    """
    n = len(pop_z)
    fitness       = np.zeros(n, dtype=np.float32)
    delta_p_all   = np.zeros(n, dtype=np.float32)
    dist_lat_all  = np.zeros(n, dtype=np.float32)
    p_target_all  = np.zeros(n, dtype=np.float32)
    ...
    for start in range(0, n, BATCH_EVAL_SIZE):
        ...
        p_target = probs[:, target_class]  # [B]

        # |Δp| = |p_c(z) - p0|
        delta_p = torch.abs(p_target - p0)  # [B]

        # Distância em latent até z0
        flat = batch_z.view(batch_z.size(0), -1)  # [B, C*8*8]
        z0_batch = z0_flat.unsqueeze(0).expand_as(flat)  # [B, C*8*8]
        dist_lat = torch.norm(flat - z0_batch, dim=1)    # [B]

        # Trust region: penaliza distâncias acima do raio
        over_radius = torch.clamp(dist_lat - TRUST_REGION_RADIUS, min=0.0)  # [B]

        # Fitness de sensibilidade (quanto maior, melhor)
        batch_fitness = (
            SENS_ALPHA_DELTA_P * delta_p
            - SENS_BETA_LAT_DIST * dist_lat
        ...
        delta_p_all[start:end]  = delta_p.cpu().numpy()
        dist_lat_all[start:end] = dist_lat.cpu().numpy()
        p_target_all[start:end] = p_target.cpu().numpy()

    return fitness, delta_p_all, dist_lat_all, p_target_all
...
def run_experiments():
    ...
    # 5.0.1: Codificar em z0 e obter p0 / classe alvo
    z0 = encode_image_to_latent(x0_pil)  # [4,8,8]
    p0, pred_class = get_classifier_prob_and_class(x0_pil)
    ...
    print(f"Classe alvo usada em LEI-Local: {CIFAR10_CLASSES[target_class]} (índice {target_class})")
    ...
    config = {
        ...
        "SENS_ALPHA_DELTA_P": SENS_ALPHA_DELTA_P,
        "SENS_BETA_LAT_DIST": SENS_BETA_LAT_DIST,
    ...
    for gen in pbar:
        # 5.3: Avaliar fitness de sensibilidade (vetorizado)
        fitness, delta_p_arr, dist_lat_arr, p_target_arr = evaluate_fitness_sensitivity(
            pop_z, z0, p0, target_class
        )
        ...
        with open(metrics_path, "a", newline="") as f:
            ...
            for idx in range(len(pop_z)):
                writer.writerow([
                    ...
                    float(fitness[idx]),
                    float(p0),
                    float(p_target_arr[idx]),
                    float(delta_p_arr[idx]),
                    float(dist_lat_arr[idx]),
                ])
    ...
    final_fitness, final_delta_p, final_dist_lat, final_p_target = evaluate_fitness_sensitivity(
        pop_z, z0, p0, target_class
    )
...
if __name__ == "__main__":
    print(f"Executando LEI-Local (Sensibilidade) com AutoencoderKL 32×32 → outputs em '{OUTPUT_BASE}'")
    run_experiments()
